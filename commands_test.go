...
func TestPikaCommand(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "PikaCommands Suite")
}

var _ = Describe("PikaCommands", func() {
	ctx := context.TODO()
	...
	Describe("keys", func() {
		It("should Del", func() {
			...
			n, err := client.Del(ctx, "key3").Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should PExpireAt", func() {
			...
			ttl := client.TTL(ctx, "key")
			Expect(ttl.Err()).NotTo(HaveOccurred())
			Expect(ttl.Val()).To(Equal(time.Duration(0)))
	...
	Describe("strings", func() {
		...
		It("should BitPosSpan", func() {
			...
			// todo 这里为啥不支持呢？
			// todo 不支持span
			//pos, err := client.BitPosSpan(ctx, "mykey", 0, 1, 3, "").Result()
			//Expect(err).NotTo(HaveOccurred())
			//Expect(pos).To(Equal(int64(16)))
			//
			//// todo 不支持span
	...
	Describe("sorted sets", func() {
		...
		It("should ZAddArgsLT", func() {
			added, err := client.ZAddLT(ctx, "zset121", redis.Z{
				Score:  2,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
			...
			vals, err := client.ZRangeWithScores(ctx, "zset121", 0, -1).Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(vals).To(Equal([]redis.Z{{Score: 2, Member: "one"}}))
		...
		It("should ZAddArgsIncr", func() {
			score, err := client.ZAddArgsIncr(ctx, "zset990", redis.ZAddArgs{
				Members: []redis.Z{
					{Score: 1, Member: "one"},
				},
			}).Result()
